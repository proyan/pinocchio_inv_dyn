...
class InvKinFormulation(object):
  name = '';
  verb = 0;
  ...
  def updateInequalityData(self, updateConstrainedDynamics=True):
    ...
    if (self.ENABLE_FORCE_LIMITS):
      ...
      ii = 0;
      for i in range(c):
        (Bfi, bfi) = self.createContactForceInequalities(self.rigidContactConstraints_fMin[i],
    ...
    if (self.ENABLE_TORQUE_LIMITS):
      ...
      self.lb = zeros(self.xdim) - 1e3;
      self.ub = zeros(self.xdim) + 1e3;
  ...
  def __init__(self, name, q, v, dt, robot):
    ...
    self.name = name;
    self.Md = zeros(
      (self.na, self.na));  # np.diag([ g*g*i for (i,g) in zip(INERTIA_ROTOR,GEAR_RATIO) ]); # rotor inertia
  ...
  def removeUnilateralContactConstraint(self, constr_name):
    ...
    if (found == False):
      ...
      if (found == False):
        raise ValueError(
          "[InvDynForm] ERROR: constraint %s cannot be removed because it does not exist!" % constr_name);
  ...
  def removeTask(self, task_info):
    if isinstance(task_info,str):
      task_name = task_info
      res = self.getTask(task_name)[0]
    else:
      res = self.tasks.index(task_info)
  ...
  def updateSupportPolygon(self):
    ...
    if (ncp == 0):
      ...
      if (np.max(np.abs(self.contact_points[2, :] - avg_z)) < 1e-3):
        ...
        (self.B_sp, self.b_sp) = compute_support_polygon(H, h, self.M[0, 0], np.array([0., 0., -9.81]),
  ...
  def setPositions(self, q, updateConstraintReference=True):
    ...
    if (updateConstraintReference):
      if (self.USE_JOINT_VELOCITY_ESTIMATOR):
        ...
        self.estimator.init(self.dt, self.JOINT_VEL_ESTIMATOR_DELAY, self.JOINT_VEL_ESTIMATOR_DELAY,
                            self.JOINT_VEL_ESTIMATOR_DELAY, self.JOINT_VEL_ESTIMATOR_DELAY, True);
  ...
  def updateConstrainedDynamics(self):
    ...
    #self.c[0:nv] = - np.dot(self.Minv, (
    #np.dot(self.Nc_T, self.h) + np.dot(self.Jc.T, np.dot(self.Lambda_c, self.dJc_v - self.ddx_c_des))));
  ...
  def computeCostFunction(self, t):
    ...
    for k in range(n_tasks):
      ...
      dim += dims[k];
    A = zeros((dim, self.nv + self.k + self.xdim));
    a = zeros(dim);
    i = 0;
    for k in range(n_tasks):
